...
export default function UserHomePage() {
    ...
    const fetchFriends = async () => {
        try {
            const token = localStorage.getItem('token');
            if (token && user) {
                const response = await axios.get(`http://localhost:3000/friends/${user.id}/friends`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
    ...
    const handleRemoveFriend = async (friendId: number) => {
        try {
            const token = localStorage.getItem('token');
            if (token && user) {
                await axios.delete(`http://localhost:3000/friends/${user.id}/remove/${friendId}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
                // After successfully removing the friend, you might want to update the friend list
                            ...
                            <ul>
                                {friends.map(request => (
                                    <li key={request.id} className="border-b border-gray-200 py-2">
                                        ...
                                        <div>
                                            <button onClick={() => handleRemoveFriend(request.id)} className="bg-red-500 text-white px-4 py-2 rounded mr-2">Remove</button>
