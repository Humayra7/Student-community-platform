...
export default function UserHomePage() {
    ...
    const fetchFriendRequests = async () => {
        try {
            const token = localStorage.getItem('token');
            if (token && user) {
                const response = await axios.get(`http://localhost:3000/friends/${user.id}/friendrequests`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
    ...
    const handleAcceptFriendRequest = async (senderId: number) => {
        try {
            const token = localStorage.getItem('token');
            if (token && user) {
                await axios.post(`http://localhost:3000/friends/${senderId}/accept`, { receiverId: user.id }, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
    ...
    const handleRejectFriendRequest = async (senderId: number) => {
        try {
            const token = localStorage.getItem('token');
            if (token && user) {
                await axios.post(`http://localhost:3000/friends/${senderId}/reject`, { receiverId: user.id }, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
                            ...
                            <ul>
                                {friendRequests.map(request => (
                                    <li key={request.id} className="border-b border-gray-200 py-2">
                                        ...
                                        <div>
                                            <button onClick={() => handleAcceptFriendRequest(request.id)} className="bg-green-500 text-white px-4 py-2 rounded mr-2">Accept</button>
                                            <button onClick={() => handleRejectFriendRequest(request.id)} className="bg-red-500 text-white px-4 py-2 rounded">Reject</button>
                                        </div>
